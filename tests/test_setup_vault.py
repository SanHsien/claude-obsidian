...
class SetupVaultTests(unittest.TestCase):
    ...
    def test_apply_supports_legacy_positional_unicode_path_and_is_idempotent(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            # Obsidian's "New link format" defaults to "shortest path when
            # possible" when unset, so links its own UI inserts are bare
            # basenames. This fork's resolver does fall back by basename and
            # reports collisions as `ambiguous_targets` rather than silently
            # picking one, so those links are not *wrong* here -- but pinning
            # the setting removes the ambiguity at the source instead of
            # detecting it afterwards.
            app_json = json.loads(
                (vault / ".obsidian" / "app.json").read_text(encoding="utf-8")
            )
            self.assertEqual(app_json.get("newLinkFormat"), "absolute")
