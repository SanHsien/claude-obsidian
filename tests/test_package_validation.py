...
class PackageValidationTests(unittest.TestCase):
    ...
    def test_shipped_vault_configs_pin_the_new_link_format(self) -> None:
        """Obsidian's own UI must insert links in the form the tools expect.

        "New link format" defaults to "shortest path when possible" when unset,
        so Obsidian inserts bare basenames. This fork's resolver does fall back
        by basename and reports collisions as `ambiguous_targets` rather than
        silently picking one -- so those links are not *wrong* here, which is
        why this is prevention rather than a fix. Pinning removes the ambiguity
        at the source instead of detecting it afterwards.

        Checked here rather than only in `test_setup_vault.py`: that file drives
        `bin/setup-vault.sh` through bash and does not run on Windows, so a
        regression in the shipped templates would go unnoticed by the fork gate.
        """
        for relative in (
            "templates/vault/.obsidian/app.json",
            "examples/sample-vault/.obsidian/app.json",
        ):
            with self.subTest(relative):
                config = json.loads((ROOT / relative).read_text(encoding="utf-8"))
                self.assertEqual(config.get("newLinkFormat"), "absolute")
