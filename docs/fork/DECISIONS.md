...
## 2026-08-30：上游 PR #182 採用（設定釘死），但前提在本 fork 不成立

**決定**：採用 `newLinkFormat: "absolute"` 到 `templates/vault/.obsidian/app.json` 與
`examples/sample-vault/.obsidian/app.json`。水位推進到 `reviewedPrThrough: 182`。

**上游的理由，以及它在這裡為什麼只對一半**：上游 PR 說「本產品每一個處理連結的地方都用精確的
vault 相對路徑解析、沒有模糊比對，所以 Obsidian 預設的 shortest-path 連結對其他工具而言無法解析，
一旦有第二個檔案同名還會**靜靜地指錯**」。

前半在本 fork 成立（設定確實沒釘），後半不成立。實查 `claude_obsidian/lint_engine.py`：

- `_LinkIndex.resolve()`（第 595 行起）先試精確路徑，**失敗時對無斜線的連結回退 `by_basename`**，
  對含路徑的連結回退後綴比對。所以 Obsidian 插入的 `[[Foo]]` 在這裡解析得到。
- 它回傳的是**候選清單**不是單一目標，同名碰撞會進 `ambiguous_targets`（第 954 行起）並列為
  lint 分類，不是靜靜選一個。
- `scripts/boundary-score.py` 更直接：註解寫明「Folder-qualified links like `[[notes/Foo]]`
  resolve to Foo.md by stem」，本來就是 stem 解析。

上游點名的另外兩個元件（resync scripts、terminology linker）本 fork 沒有。

**所以這是預防不是修錯**：本 fork 不會因為那個設定沒釘而指錯連結，但釘了可以從源頭消除歧義，
而不是事後偵測。兩行設定、零風險，採用。

**測試放哪**：上游把測試放 `tests/test_setup_vault.py`（驗**產生出來的** vault）——那份保留，
但它會透過 bash 執行 `bin/setup-vault.sh`，在 Windows 跑不起來（本機 10 條全紅，與本次改動無關），
也不在 `tools/dev_check.ps1` 的 portable 清單裡。所以另外在
`tests/test_package_validation.py` 加一條驗**出貨的樣板檔**——那份在 fork gate 裡真的會跑。
兩者驗的是不同性質：輸入樣板 vs 產出結果。

**證據**：fork gate 全綠（overlay pytest 46 passed、五份 portable test 全過、package validate、
連結檢查）。

**觸發條件**：若哪天把 `resolve()` 的 `by_basename` 回退拿掉、改成只認精確路徑，上游那句「靜靜
指錯」就會在這裡成立，那時要重讀這條。
